...
const handleDelete=async(id)=>{
    ...
    return (
      <AppLayout>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
          <Typography sx={{color:"white"}}>データが存在しません</Typography>
        </Box>
      </AppLayout>
    );
  }
  return (
         ...
         <Box
          // onClick={handleOpen}
           ...
           <Grid item xs={10} md={6} 
             ...
             objectFit:"contain",}} 
             src={path}/>
             <Typography varient="h6" component={"div"} sx={{color:"white", marginTop:"20px",marginBottom:"15px"}}noWrap>{name}</Typography>
             <Rating 
      ...
      <Grid container spacing={2} sx={{display: 'flex', alignItems: 'stretch' ,marginTop:"50px" ,padding:"40px"}}>
      {activeSquads.map((player)=>(
        <Grid item key={player.id} xs={4} lg={1.5}
      ...
      <Card sx={{borderRadius:"3%",height:"100%"}}>
        <CardActionArea sx={{height:"100%"}}>
          ...
          <ImageLoader src={player.photo} style={{display:"inlineBlock",width:"100%",textAlign:"center"}}/>
          <CardContent sx={{width:"100%"}}>
           <Typography varient="h6" component={"div"} sx={{ textAlign:"center",width:"100%",textOverflow: "ellipsis", overflow: "hidden", }}>{player.name}</Typography>
          </CardContent>  
          </Box>
          {/* </Link> */}
        </CardActionArea>
    ...
    sx={{
      ...
      variant="h6"
      component="h2"
      sx={{textAlign:"center",fontSize:{xs:"15px",sm: "1.25rem"}}}>
        チームを評価する
      </Typography>
      <Box sx={{paddingTop:"20px",display:"flex",flexDirection:"column",justifyContent:"space-around"}}>
      <Rating required 
    ...
    <Modal open={detailOpen} onClose={detailHandleClose}>
      <Box 
         sx={{
          ...
          />
          <ImageLoader src={playerDetail.photo} style={{borderRadius:"50%"}}/>
          <CardContent sx={{display:"flex",flexDirection:"column",alignItems:"center",marginTop:"20px"}}>
            <Typography varient="h6" component={"div"} noWrap>name:&nbsp;{playerDetail.name}</Typography>
            <Typography varient="h6" component={"div"} noWrap>position:&nbsp;{playerDetail.position}</Typography>
            <Typography varient="h6" component={"div"} noWrap>age:&nbsp;{playerDetail.age}</Typography>
            <Typography varient="h6" component={"div"} noWrap>number:&nbsp;{playerDetail.number}</Typography>
          </CardContent> 
          </Box>
          {/* </Link> */}
      {/* </Card>  */}
...
export async function getServerSideProps(context){
  const {teamId}=context.params;
    try{
    const apiUrl = 'https://api-football-v1.p.rapidapi.com/v3/players/squads';
